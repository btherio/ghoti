...
if(isset($_GET) && $_GET['apikey'] == $apikey){    // check get variable, and check apikey matches. This is our only rudimentary security
    ...
    if(isset($_GET['sensorip']) && isset($_GET['data'])){   // check additional variables
    //get sensor by ip
        ...
        if(count($sensor, COUNT_NORMAL) < 1){ //tryna check if we got a sensor id back, if not this sensor isnt in the db.
            echo "Server: Sensor lookup failed. Is this sensor added to your list?";
            ghoti::log("sensorPing: Failed to find sensor from". $_GET['sensorip']);
        } else {
            $id = 0;
            ...
            if($sensor['type'] === "relay"){  //if this sensor is a relay, we want to sync our actual relay status (what it is) with the database relay status (what it should be)
                echo "Server: Found wifi relay reporting, checking relay state in db...\n";
                $relay = $relays->getRelayState($sensor['address']);
                $dbState = $relay[0]; //we pulled out the relay id and the db state from that function
                $relayID = $relay[1];
                if(($dbState === "on" && $data == 0)||($dbState === "off" && $data == 1)){ //dbstate Mismatchs
                    echo "Server: $dbState:$data found db state mismatch, trying to fix";
                    $relays->switchRelay($relayID,1,$dbState,$sensor['address']);
                } else {
                    echo "Server: $dbState:$data found no db state mismatch\n";
                }
            }
            ...
            if($id > 0){ //this prevents unwanted additions.
                ...
                $sensorsdb->addSensorData($id,$unixTime,round($data,1),round($auxData,1)); //save sensor data, rounding off the sensor data, id,unixtime(date),data
                    ...
                    foreach($setpoints as $setpoint){
                        if($setpoint[2] == "HIGH"){
                            ...
                            if($data >= $setpoint[1]){ //if we have hit or surpassed our setpoint
                                echo " is reached. ";
                                if($setpoint[3] === "1" || $setpoint[3] === "2" ){ //check for alarm
                                    //do alarm code
                                    echo "ALARM\n";
