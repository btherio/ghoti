...
unset ghoti_admin_password</code></pre>
      ...
      <section id="docs-features" class="ghotiGuideSection">
        ...
        <div>
          ...
          <div class="ghotiGuideFeatureList">
            ...
            <article><h3>Send Email</h3><p>Write one plain-text message and send it to a single user, a selection of users, or everyone with a valid address. Each recipient gets their own copy, so nobody sees anyone else&rsquo;s address, and one rejection does not stop the rest; the result line reports how many were sent, skipped for a missing address, and failed. The message is styled with the site&rsquo;s default theme colours and fonts, read from that theme&rsquo;s stylesheet, and is also sent as plain text. Requires enabled Mail Settings, and sends at most 200 recipients per message.</p></article>
            <article><h3>Analytics</h3><p>Three tabs. <b>Usage data</b> covers pageviews, sessions, pages, browsers, devices and referrers; visitors are tracked only after consent, and Exclude admin views gives visitor-only numbers. <b>Errors</b> groups and counts error-like lines from this site’s own log, so a recurring fault stands out from a one-off. <b>Server logs</b> holds the raw log and the <b>Apache logs</b> analyser for the web server’s own access and error logs — see the <a href="docs/apache-log-analyzer.md" target="_blank" rel="noopener noreferrer">Apache log analyzer guide</a>. The range buttons and the CSV export scope the first two tabs.</p></article>
      ...
      <section id="docs-operations" class="ghotiGuideSection">
        ...
        <div>
          ...
          <p>Delivery depends on mail. Once mail is enabled and a test message has succeeded, each export is emailed as an attachment to every administrator and browser downloads are disabled, including direct requests to old download URLs; until then the exports are downloadable. The SQL dump contains password hashes and may contain SMTP credentials, so treat those messages and their mailboxes accordingly. See the <a href="docs/backups.md" target="_blank" rel="noopener noreferrer">backup delivery notes</a>.</p>
          <h3>Updates</h3>
