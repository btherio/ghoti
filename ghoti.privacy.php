...
/*
 * Named disclosure of the ad network, added only while ads are actually being
 * served. A generic "the site displays advertising" paragraph is true of every
 * install; naming Google, and naming what the visitor can do about it, is only
 * honest on an install that has switched Google's tag on.
 */
function ghoti_privacy_advertising(){
    if(!isset($_SESSION['bannersObj']) || !($_SESSION['bannersObj'] instanceof banners)){ return ''; }
    if(!$_SESSION['bannersObj']->adsActive()){ return ''; }
    return '<p>This site currently serves advertising through <strong>Google AdSense</strong>. Google receives your IP address, browser information, and the page you are viewing in order to select an ad, and may set or read its own cookies and similar identifiers under <a href="https://policies.google.com/technologies/partner-sites">Google’s own privacy notice</a>. You can review and change ad personalization at <a href="https://myadcenter.google.com/">My Ad Center</a>. When your browser sends a Global Privacy Control or Do Not Track signal, this site requests non-personalized ads for you; Google still needs the technical information above to serve any ad at all. This site does not operate a consent management platform, so visitors in jurisdictions that require prior opt-in consent for advertising cookies should not be served ads from here without one.</p>';
}
