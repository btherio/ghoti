...
class loginui{
	...
	public function printSystemMenu(){
		//$this->output .= "<ul>\n";
		$this->output .= "<li class=\"nav-item\"><a href=\"#\" class=\"ghotiMenu\" onclick=\"logout();\">Log Out</a></li>\n";
		$this->output .= "<li class=\"nav-item\"><a href=\"#\" class=\"ghotiMenu\" onclick=\"printChangePasswordForm();\">Change Password</a></li>\n";
	...
	public function printPopupLogin(){
		$this->output = "<ul id=\"ghotiLogin\"><a href=\"#\" onclick=\"popupLogin();\">Login</a></ul>\n";
		return $this->output;
	}
	...
	function printManageUserForm($userList){
		...
		foreach($userList as $records => $row){
			$this->output .= "<tr><td><input type=\"text\" id=\"".$row[1]."\" value=\"".$row[1]."\" size=\"10\" /></td>\n";
			...
			$this->output .= "<input type=\"button\" value=\"Delete\" onclick=\"deleteUser('".$row[0]."');\" /></tr>\n";
		}
		$this->output .= "</table>\n";
		return $this->output;
	}
