...
function readSensorsFromDB(){
    /**
     * Read sensordata from database.
     * This allows unreachable sensors to still display their last known data.
     * It's also way fuckin faster to read a database than to send a curl request to each IP address.
     *
     *
    */
    ...
    try{
        ...
        foreach($result[0] as $k => $x){
            ...
            if($sensorData[$k][6]){
                ...
                $htmlSensorAuxData = $sensorData[$k][6][0][2]; //sensor aux data
                $htmlDataDate = $sensorData[$k][6][0][0]; //sensor data date
                $unixTimestamp = strtotime($htmlDataDate." MDT");
                $unixCurrent = strtotime(exec('date'));
                $dataAge = ($unixCurrent - $unixTimestamp);
                if($dataAge > 300){ //timestamp is older than 5min
                    $sensorData[$k][7] = 0; //sensor offline
                } else {
                    $sensorData[$k][7] = 1; //sensor online
                }
                $sensorData[$k][5] = "<br /><span alt=\"$htmlDataDate\">".$htmlSensorData." ".$sensorData[$k][4].""; //tease out the sensors data.
                if($htmlSensorAuxData != 0){
                    $sensorData[$k][5] .= "<br /><sub>($htmlSensorAuxData)</sub>";
                }
                $sensorData[$k][5] .= "</span>";
            }
            else{
                $sensorData[$k][5] = "No data...";
            }
            if($_SESSION["sensorsObj"]->sensorsdb->checkAlarms($sensorData[$k][0])){
                //we have an alarm
                $sensorData[$k][8] = 1;
            }else{
                $sensorData[$k][8] = 0;//no alarm
            }
        }
    } catch(Exception $e){
        ghoti::log("sensors.ajax.php: $e\n");
...
function getSensorDataByIdAllTime($id){
    $sensorData = $_SESSION["sensorsObj"]->sensorsdb->getSensorDataByIdAllTime($id);
    return $sensorData;
}
sajax_export("getSensorDataByIdAllTime");
