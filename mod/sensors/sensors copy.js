...
function printSensorsForm(result){
    ...
    for (x in sensorsArray){
        ...
        if((x % 2) == 0 || x == 0){
            //comment this line out for a single vertical column, otherwise you get 2 columns
            formContent += "<div class=\"flex-container\">";
        }
        formContent += "<div class=\"container-fluid\"><div class=\"card shadow\"><div class=\"card-header py-3\">";
        ...
        if(sensorsArray[x][7] == 1){ //check if sensor has returned online or offline
            formContent += "<img src=\"mod/sensors/connect.png\" height=\"24\" width=\"24\" />";
        } else {
            formContent += "<img src=\"mod/sensors/disconnect.png\" height=\"24\" width=\"24\" />";
        }
        formContent += "<label alt=\"name\" id=\""+sensorID+"-name\"><b>"+sensorName+"</b></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";

        if(sensorsArray[x][8] == 1){ //check if sensor has current alarms
            formContent += "<img src=\"gfx/alarm.png\" style=\"float: right\" height=\"22\" width=\"22\" />";
        }else{
            formContent += "";
        }

        formContent += "</div><div class=\"card-body\">";

        formContent += "<label id=\""+sensorID+"-address\">"+sensorAddress+"</label>&nbsp;&nbsp;&nbsp;";
        formContent += "<label id=\""+sensorID+"-type\">"+sensorType+"</label>&nbsp;&nbsp;&nbsp;<br />";
        formContent += "<a href=\"#\" class=\"ghotiMenu\" onclick=\"manageSetpoints("+sensorID+")\" >Setpoints</a>&nbsp;&nbsp;&nbsp;&nbsp;";
        formContent += "<a href=\"#\" class=\"ghotiMenu\" onclick=\"clearSensorData("+sensorID+")\" >Clear-Data</a>&nbsp;&nbsp;&nbsp;&nbsp;";
        formContent += "<a href=\"#\" class=\"ghotiMenu\" onclick=\"modifySensorForm("+sensorID+",'"+sensorName+"','"+sensorAddress+"')\" >Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;";
        formContent += "<a href=\"#\" class=\"ghotiMenu\" onclick=\"deleteSensor("+sensorID+")\" >Delete</a>";
        //formContent += "<br /><br /><br />";
        formContent += "</div></div></div>";
        if((x % 2) != 0 ){
            //as above, columns of two, end the column
            formContent += "</div>";
        }
...
function printSensorsOverview(result){
    ...
    for (x in sensorsArray){
        ...
        liveContent += "<div class=\"container-fluid\"><div class=\"card shadow\"><div class=\"card-header py-3\">";
        liveContent += "<input type=\"hidden\" id=\""+sensorID+"-id\" value=\""+sensorID+"\" />";
        liveContent += "<h5>";
        if(sensorsArray[x][7] == 1){ //check if sensor has returned online or offline
            liveContent += "<img src=\"mod/sensors/connect.png\" height=\"24\" width=\"24\" alt=\""+sensorDate+"\" />";
        } else {
            liveContent += "<img src=\"mod/sensors/disconnect.png\" height=\"24\" width=\"24\" alt=\""+sensorDate+"\" />";
        }

        liveContent += "<a href=\"#\" class=\"ghotiMenu\" alt=\""+sensorDate+"\" onclick=\"x_getSensorDataById("+sensorID+",printSensorData)\">"+sensorName+"</a>";

        if(sensorsArray[x][8] == 1){ //check if sensor has current alarms
            liveContent += "<img src=\"gfx/alarm.png\" style=\"float: right;\" height=\"22\" width=\"22\" />";
        }else{
            liveContent += "";
        }

        liveContent += "</h5></div><div class=\"abox card-body\" style=\"text-align: center;\">"+sensorData+"<br /><br /></div></div></div><br /><br />";
    ...
    if(("#liveContent").length){
        ...
        window.setTimeout('readSensors()',30000); //loop it

        //window.setTimeout('x_readSensors(printSensorsOverview)',20000); //loop it
    }

}
function printSensorsOverviewOLD(result){
    sensorsArray = result;
    liveContent = "";
    ...
    for (x in sensorsArray){
        ...
        liveContent += "<div class=\"container-fluid\"><div class=\"card shadow\"><div class=\"box\"><div class=\"card-header py-3\">";
        liveContent += "<input type=\"hidden\" id=\""+sensorID+"-id\" value=\""+sensorID+"\" />";
        liveContent += "<h6>";
        if(sensorsArray[x][7] == 1){ //check if sensor has returned online or offline
        ...
        liveContent += "<a href=\"#\" class=\"ghotiMenu\" alt=\""+sensorDate+"\" onclick=\"x_getSensorDataById("+sensorID+",printSensorData)\">"+stripslashes(sensorName)+"</a></h6>";
        liveContent += "</div><div class=\"box card-body\">"+sensorData+"</div></div></div></div>";
    ...
    if(("#liveContent").length){
        x_getRelays(900,printRelaysOverview);// add relays
        $("#liveSensors").html(liveContent); //write the content to the page
        window.setTimeout('readSensors()',2000); //loop it
