...
function getSensorDataByIdAllTime($sensorID){
	try{
		$Query = $this->adodb->GetArray("select sensorData.id,sensorData.date,sensorData.data,sensors.name,sensors.type,sensors.units from sensorData,sensors  where sensorData.id=sensors.id and sensorData.id=? order by date;",array($sensorID));
		if (!$Query) ghoti::log($this->adodb->ErrorMsg());
	}catch (exception $e){
		ghoti::log("sensors.db.php $e");
		return False;
...
public function getLatestData($id){
	try{
		$sensors = $this->adodb->GetArray("select date,data,auxData from sensorData where id = ? order by date desc limit 1;",array($id));
}catch (exception $e){
		ghoti::log("sensors.db.php $e");
		return $e->getMessage();
	}
...
//maybe	// select sensors.name,sensorSetpoints.setpoint,sensorSetpoints.type,relays.name from sensors,sensorSetpoints,relays where sensorSetpoints.id=relays.id && sensorSetpoints.id=sensors.id;
		...
		$setpoints = $this->adodb->GetArray("select relays.id as 'relayID',sensorSetpoints.setpoint,sensorSetpoints.type,sensorSetpoints.action,sensorSetpoints.id as 'sensorID',relays.name as 'action',sensors.name from sensorSetpoints,relays,sensors where sensorSetpoints.action = relays.id AND sensorSetpoints.id = sensors.id AND sensorSetpoints.id = ?",array($id));
	}catch (exception $e){
		ghoti::log("sensors.db.php $e");
		return $e->getMessage();
	}
...
function setAlarmSent($id,$setpoint){
	try{
		$nonQuery = $this->adodb->Execute("update sensorSetpoints set action=2 where action=1 and id=? and setpoint=?",array($id,$setpoint));
		if (!$nonQuery) ghoti::log($this->adodb->ErrorMsg());
	}catch (exception $e){
		ghoti::log("sensors.db.php $e");
		return False;
...
function checkAlarms($id){
	try{
		$query = $this->adodb->Execute("select * from sensorSetpoints where action=2 and id=?",array($id));
		if (!$query) ghoti::log($this->adodb->ErrorMsg());
	...
	//if($query->fields[0] > 0){ //if number of records returned is greater than 0
	if($query->fields){ //if number of records returned is greater than 0
		return True; //we have an alarm
	}
	return False; //if we made it this far, no alarm
}
}
?>
