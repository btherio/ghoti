...
class relaysdb extends ghotidb{
    ...
    function addRelay($name,$pin,$state="off",$address="local"){
		try{
	...
	public function getRelayByAddress($address){
        try{
            $relays = $this->adodb->GetArray("select id,name,pin,state,address from relays where address=?;",array($address));
		}catch (exception $e){
			ghoti::log("relays.db.php $e");
			return $e->getMessage();
		}
